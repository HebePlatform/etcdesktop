        ...
        <div id="container">
            <div id="output">
                <div class="containerT">
                    <div style="max-width: 600px;margin: 0 40px;background: rgba(10, 10, 10, 0.17);padding: 25px;box-sizing: border-box;justify-content: center;border-bottom: 8px solid rgb(82, 194, 52); border-radius: 30px 30px 50px 50px;">
                        <div>
                            <img src="../../assets/3.png" style="width: 80px;padding-top: 10px;padding-bottom:10px"/>
                        </div>
                        <div style="color: #fff;text-align: center;font-size: 24px">
                            Login Etc Wallet
                        </div>
                        <form class="form">
                            <textarea v-model="prv" placeholder="Please enter Privatekey">
                            </textarea>

                            <el-input show-password  v-model="pass" type="password" placeholder="Please enter password"/>
                            <el-input show-password  v-model="pass1" type="password" placeholder="Please enter the password again"/>

                            <button type="button"
                                    @click="importwallet"
                                    style="font-weight: normal;margin-top: 10px">Import Wallet
                            </button>
                            <div @click="createwallet" class="createlogin" style="margin-top: 14px">
                                Don't have a wallet? Click here to create one!
                            </div>
                        </form>
                    </div>
                </div>
    ...
    export default {
        ...
        methods: {
            importwallet() {
                ...
                if (this.prv == '') {
                    this.$message.error('Privatekey cannot be empty');
                    return;
                }
                if (this.pass == '') {
                ...
                let addr = this.$g.address(this.prv)
                if (addr == '') {
                    this.$message.error('Wrong private key');
                    return;
                }
                let encrypt = this.$g.encrypt(this.prv, this.pass1)

                this.$g.stores.set('wallet', {
                    address: addr,
                    wallet: [
                        {
                            address: addr,
            ...
            createwallet() {
                this.prv = this.$g.create().privateKey
            },
            init() {
                var CAV = {FRONT: 0, BACK: 1, DOUBLE: 2, SVGNS: "http://www.w3.org/2000/svg"};
                ...
                CAV.Plane = function (a, b, c, d) {
                    ...
                    for (a = 0; a < this.segments; a++) for (b = 0; b < this.slices; b++) d = c[a + 0][b + 0], e = c[a + 0][b +
